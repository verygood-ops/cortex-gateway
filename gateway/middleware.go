...
func init() {
	...
	flag.StringVar(&jwtValidationAlgo, "gateway.auth.jwt-validation-algo", SigningAlgoHMAC, "JWT Validation algorithm: HMAC or RSA")
}
...
var AuthenticateTenant = middleware.Func(func(next http.Handler) http.Handler {
	...
	// Prepare key parsing algorithms
	...
	switch algoLower {
		...
		default:
			logger := log.With(util.Logger)
			level.Info(logger).Log("Using default string key format")
	}
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch algoLower {
			case SigningAlgoRSA:
				err = jwtToTenant(r, te, logger, SigningAlgoRSA, &jwtRsaKey)
			case SigningAlgoHMAC:
				err = jwtToTenant(r, te, logger, SigningAlgoHMAC, jwtSecretBytes)
			default:
				level.Warn(logger).Log("msg", "invalid signing algorithm used")
